...
export function NotificationsPage() {
    ...
    const userNotifications = notifications
        .filter(n => n.userId === user?.id && n.role === 'customer')
    ...
    return (
        <div className="bg-slate-50 dark:bg-zinc-950 min-h-screen font-display pb-10">
            ...
            {userNotifications.length > 0 && (
                <div className="sticky top-[53px] z-40 bg-slate-50/80 dark:bg-zinc-950/80 backdrop-blur-md px-6 py-2.5 flex justify-between items-center border-b border-slate-200/50 dark:border-zinc-800/50">
                    <button
                        onClick={() => user?.id && markAllAsRead(user.id, 'customer')}
                    ...
                    <button
                        onClick={() => {
                            if (window.confirm("Clear all notifications?")) {
                                user?.id && clearAll(user.id, 'customer')
                            }
                        }}
            ...
            <div className="p-4 flex flex-col gap-4">
                {userNotifications.length === 0 ? (
                    ...
                    userNotifications.map(notif => (
                        <div
                            ...
                            className={`bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border transition-shadow cursor-pointer relative ${notif.isRead
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-1">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${notif.type === 'booking' ? 'bg-blue-100 text-blue-700' :
                                        notif.type === 'payment' ? 'bg-green-100 text-green-700' :
                                            notif.type === 'quote' ? 'bg-purple-100 text-purple-700' :
                                                'bg-slate-100 text-slate-700'
                                        }`}>
                                        {notif.type}
                                    </span>
                                </div>
                                <h4 className={`font-bold text-slate-900 dark:text-white text-base pr-4 ${!notif.isRead ? "" : "opacity-80"}`}>
                                    {notif.title}
                                </h4>
                ...
                <div className="text-center py-8">
                    <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">End of Notifications</p>
                </div>
            </div>
        </div>
    )
